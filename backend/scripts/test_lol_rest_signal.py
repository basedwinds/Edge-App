...
Elo alone. See test_cs2_rest_signal.py's own docstring for the shared
rationale and the two competing real hypotheses this tests.

Mirrors elo_lol.py::update_ratings exactly for the underlying rating walk
(per-map updates using the real maps_won_a/maps_won_b split, same as
derive_lol_elo_constants.py/test_lol_h2h_signal.py) -- the rest adjustment
only touches the PREDICTION for a match, never the persistent Elo state."""
...
from app.models.baseline.elo_lol import BASE_RATING, K as SHIPPED_K, map_win_prob, series_score_distribution  # noqa: E402
...
def main():
    ...
    for points_per_day in (40, 50, 60, 70, 80, 100, 120):
        for cap_days in (1,):
