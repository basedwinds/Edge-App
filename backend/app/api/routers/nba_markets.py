...
def list_nba_futures(session: Session = Depends(get_session)):
    ...
    for m in markets:
        ...
        stake_dollars = size_stake_dollars(staking_mode, kelly, futures_pool, model_prob, implied, unit_dollars, flat_marginal, flat_full, unit_scale=FUTURES_UNIT_SCALE, min_market_price=FUTURES_MIN_MARKET_PRICE, sport="nba")  # every FUTURES_MARKET_TYPES entry is a futures-pool market
        out.append(
            FuturesMarketOut(
                id=m.id,
