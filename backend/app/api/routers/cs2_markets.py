...
def list_cs2_futures(session: Session = Depends(get_session)):
    ...
    for m in markets:
        ...
        _stake = size_stake_dollars(_mode, _kelly, _futures_pool, model_prob, implied, _unit, _fm, _ff, unit_scale=FUTURES_UNIT_SCALE, min_market_price=FUTURES_MIN_MARKET_PRICE, max_spread=FUTURES_MAX_SPREAD, yes_bid=snap.yes_bid if snap else None, yes_ask=snap.yes_ask if snap else None, sport="cs2", team=m.team)
...
def list_cs2_markets(session: Session = Depends(get_session)):
    ...
    _borrowed = borrowed_start_times(session.query(Cs2Match).all())
    _fixture_keys = canonical_fixture_ids(session, Cs2Match)
    ...
    for m in markets:
        ...
        if kelly is None:
            _no = no_side_inputs("cs2", m.market_type, model_prob, implied,
        ...
        stake_dollars = size_stake_dollars(staking_mode, kelly, pool, _smp, _smk, unit_dollars, flat_marginal, flat_full, max_spread=FUTURES_MAX_SPREAD, yes_bid=_sbid, yes_ask=_sask,  unit_scale=_uscale, sport="cs2", team=m.team)
        ...
        out.append(
            Cs2MarketOut(
                ...
                fixture_key=_fixture_keys.get(m.cs2_match_id, m.cs2_match_id),
