...
from app.models.esports_tournament_pricing import find_event_path, is_competition_outcome, price_tournament_winners
...
def list_valorant_futures(session: Session = Depends(get_session)):
    ...
    for m in markets:
        ...
        _stake = size_stake_dollars(_mode, _kelly, _futures_pool, model_prob, implied, _unit, _fm, _ff, unit_scale=FUTURES_UNIT_SCALE, min_market_price=FUTURES_MIN_MARKET_PRICE, sport="valorant", team=m.team)
...
def list_valorant_markets(session: Session = Depends(get_session)):
    ...
    for m in markets:
        ...
        stake_dollars = size_stake_dollars(staking_mode, kelly, pool, model_prob, implied, unit_dollars, flat_marginal, flat_full, unit_scale=_uscale, sport="valorant", team=m.team)
