...
def list_markets(session: Session = Depends(get_session)):
    ...
    for m in markets:
        ...
        # CAPS ONLY BIND IF SOMEONE ASKS FOR THEM. exposure.py defines a 60%
        # total ceiling (20% futures / 40% game), a per-sport share and a
        # per-team futures share, and size_stake_dollars has accepted
        # `remaining_capacity` all along -- but NO router passed it, so every one
        # of those ceilings was inert. Measured 2026-08-11: NFL futures sized $75
        # against a $26.40 sub-pool, ~3x over, because flat mode sizes by unit
        # tier and nothing trimmed it afterwards.
        #
        # The side is inferred from _uscale inside remaining_for_unit_scale, so
        # the same call is correct on both the game and futures paths.
        _cap = exposure.remaining_for_unit_scale(_uscale, sport="nfl", team=m.team)
        stake_dollars = size_stake_dollars(staking_mode, kelly, pool, model_prob, implied, unit_dollars, flat_marginal, flat_full, unit_scale=_uscale, min_market_price=_minpx, sport="nfl", team=m.team, remaining_capacity=_cap)
        out.append(
            MarketOut(
                id=m.id,
