...
| 2026-08-15 | Futures should be blocked once the event/season has started | **BACKWARDS for everything except esports** | Real-sport season sims already REPLAY played games (CFB "games already played contribute their real result", racing "starting from the CURRENT standings", NBA/MLB/WNBA simulate only remaining) and tennis starts from the deepest resolved round. Blocking them mid-season would delete most of the board and make the model WORSE. Verified Valorant is progress-aware too: 12-team fields into a confirmed 8-slot playoff off live vlr.gg standings | `#207` |
| 2026-08-15 | "Single event" vs "season-long league" is a usable signal for gating | **NO — it does not exist in the data** | A name-free detector (two field members already played) fires on ALL 33 groups, because without event_name the join is not scoped to THIS event: LPL teams meet all season, EWC teams meet elsewhere too. Span does not separate either (BLAST Porto 12d < LCS Summer 15d). Real discriminator is has-progress-data-vs-not | `#207` |
