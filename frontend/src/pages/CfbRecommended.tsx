...
import {
  ...
  fetchPlacedBets,
  markBetPlaced,
  buildCfbRecommendedBets,
  crossPlatformKey,
  type RecommendedBetRow,
} from "../api/markets";

export function CfbRecommended() {
  const queryClient = useQueryClient();
  const marketsQuery = useQuery({ queryKey: ["cfb", "markets"], queryFn: fetchCfbMarkets });
  const settingsQuery = useQuery({ queryKey: ["settings"], queryFn: fetchSettings });
  const lockedQuery = useQuery({ queryKey: ["locked-pools", "cfb"], queryFn: () => fetchLockedPools("cfb") });
  const pendingBetsQuery = useQuery({
    queryKey: ["placed-bets", "cfb", "pending"],
    queryFn: () => fetchPlacedBets("pending", "cfb"),
  });
  ...
  const result = useMemo(
    () =>
      buildCfbRecommendedBets(
        marketsQuery.data ?? [],
        settingsQuery.data?.cfb_weekly_pool_dollars ?? 0,
        settingsQuery.data?.cfb_futures_pool_dollars ?? 0,
  ...
  async function handleMarkPlaced(row: RecommendedBetRow) {
    ...
    queryClient.invalidateQueries({ queryKey: ["placed-bets", "cfb"] });
    queryClient.invalidateQueries({ queryKey: ["locked-pools", "cfb"] });
  }

  return (
    <PageShell title="College Football Recommended Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        College Football moneyline only (KXCollege FootballGAME). A bet appears here only if quarter Kelly (capped at 5% of the
        College Football pool per position — see Settings) computes a positive stake, which requires at least a 3pp
        edge. It's a mechanical filter on unvalidated model output (model_validated: false — the College Football Elo
