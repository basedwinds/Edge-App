...
export function NbaPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "nba"],
    queryFn: () => fetchPlacedBets(undefined, "nba"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["nba", "markets"] });
    queryClient.invalidateQueries({ queryKey: ["nba", "futures"] });
  }
  ...
  return (
    <PageShell title="NBA Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Moneyline/spread/total auto-settle once the real final score lands. Futures/season-long NBA
        markets need the Won/Lost/Push buttons above once you know the outcome. A pending bet's stake
        stays locked out of NBA's own pool budget on the Recommended Bets page until it's settled.
      </p>
    </PageShell>
  );
}
