...
/** LoL's own dedicated Placed Bets page -- see ValorantPlaced.tsx's own
 * docstring for the shared "manual settlement only" precedent. */
export function LolPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "lol"],
    queryFn: () => fetchPlacedBets(undefined, "lol"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["lol", "markets"] });
  }
  ...
  return (
    <PageShell title="LoL Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        LoL bets do not auto-settle yet (no bet_settlement.py support built for any esports title) -- use
        the Won/Lost/Push buttons above once you know the outcome. A pending bet's stake stays locked out
        of LoL's own pool budget on the Recommended Bets page until it's settled. CLV (closing-line value)
        IS computed for match-tied (weekly-pool) bets -- Leaguepedia's own real DateTime_UTC field gives a
        real single closing moment, same as Tennis/Soccer. Futures bets show current-price-vs-placement
