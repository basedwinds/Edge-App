...
export function ValorantCalibration() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["bet-stats", "valorant"],
    queryFn: () => fetchBetStats("valorant"),
  });

  const brierBetter = data && data.brier_score !== null && data.market_brier_score !== null && data.brier_score < data.market_brier_score;

  return (
    <PageShell title="Valorant Calibration">
      {isError && (
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex flex-wrap border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] mb-6 divide-x divide-[var(--color-border)]">
            ...
            <StatTile
              label="Avg. CLV"
              value={data.avg_clv_pp !== null ? `${(data.avg_clv_pp * 100).toFixed(1)}pp` : "—"}
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- consistent with this app's own backtest (see Backtests), which found the market beats this Elo baseline on Valorant's own real Kalshi trade history."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        ...
        much smaller sample. This app's own real market-odds backtest (see Backtests) already found the
        market beats this Elo baseline on Valorant's own real Kalshi trade history -- none of this is a
        claim that any model here beats the market.
      </p>
    </PageShell>
  );
}
