...
import { fetchWnbaFutures } from "../api/markets";
import { useFuturesMarkPlaced } from "../hooks/useFuturesMarkPlaced";

export function WnbaFutures() {
  const { data, isLoading, isError } = useQuery({ queryKey: ["wnba", "futures"], queryFn: fetchWnbaFutures });
  const onMarkPlaced = useFuturesMarkPlaced("wnba");
  const rows = data ?? [];
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex flex-wrap border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] mb-6 divide-x divide-[var(--color-border)]">
            <StatTile label="Market types" value={String(stats.typeCount)} sublabel="championship / conference / playoff qualifier / one seed / win total" />
          ...
          <FuturesTable rows={rows} onMarkPlaced={onMarkPlaced} sport="wnba" />
        </>
      )}

      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Priced by the WNBA season simulation, run forward from current Elo with a team-strength sigma of
        100 fitted against real season outcomes. Championship, conference, playoff-qualifier and
        one-seed markets all resolve off the same simulated standings, so they stay mutually consistent.
        model_validated is false: the sigma is fitted, but no WNBA future has settled yet to score the
        model against the market. One thing this simulation cannot see is availability &mdash; a season
        that loses a key player mid-way will drift from it until the Elo ratings catch up.
      </p>
    </PageShell>
  );
}
