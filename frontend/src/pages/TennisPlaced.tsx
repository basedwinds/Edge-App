...
export function TennisPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "tennis"],
    queryFn: () => fetchPlacedBets(undefined, "tennis"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["tennis", "markets"] });
  }
  ...
  return (
    <PageShell title="Tennis Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Tennis bets do not auto-settle yet (no bet_settlement.py support built for Tennis) -- use the
        Won/Lost/Push buttons above once you know the outcome. A pending bet's stake stays locked out of
        Tennis's own pool budget on the Recommended Bets page until it's settled. CLV (closing-line
        value) isn't computed for Tennis yet either -- no closing-price snapshot capture is built for this
        sport yet, so this shows current-price-vs-placement instead, same honest degrade other sports use
        where real closing data isn't available.
      </p>
    </PageShell>
  );
}
