  ...
  return (
    <PageShell title="MMA Calibration">
        ...
        <>
          <div className="flex flex-wrap border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] mb-6 divide-x divide-[var(--color-border)]">
            ...
            <StatTile
              label="Avg. CLV"
              value="—"
              sublabel="not computed for MMA yet -- no single per-fight closing moment this app tracks, see Placed Bets"
            />
          </div>
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- the moneyline/went-the-distance backtests already found no free historical UFC odds source to validate against (see Backtests), so live tracking here is the real test."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Win rate needs many settled bets before it means much. Brier score is usable with a much smaller
        sample -- for MMA specifically, live tracking here is the real market-quality check, since no free
        historical UFC odds source exists to backtest against (see Backtests). None of this is a claim
        that any model here beats the market.
      </p>
    </PageShell>
  );
}
