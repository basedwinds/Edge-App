...
export function CfbPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "cfb"],
    queryFn: () => fetchPlacedBets(undefined, "cfb"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["cfb", "markets"] });
  }
  ...
  return (
    <PageShell title="College Football Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        College Football moneyline auto-settles once the real final score lands. A pending bet's stake stays locked out
        of College Football's own pool budget on the Recommended Bets page until it's settled.
      </p>
    </PageShell>
  );
}
