...
/** Valorant's own dedicated Placed Bets page -- parallel to
 * TennisPlaced.tsx/SoccerPlaced.tsx. No auto-settlement built for any
 * esports title yet (no bet_settlement.py support), same honest "manual
 * only" precedent Tennis/MMA already established -- use the Won/Lost/Push
 * buttons once you know the real outcome. */
export function ValorantPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "valorant"],
    queryFn: () => fetchPlacedBets(undefined, "valorant"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["valorant", "markets"] });
  }
  ...
  return (
    <PageShell title="Valorant Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Valorant bets do not auto-settle yet (no bet_settlement.py support built for any esports title) --
        use the Won/Lost/Push buttons above once you know the outcome. A pending bet's stake stays locked
        out of Valorant's own pool budget on the Recommended Bets page until it's settled. CLV
        (closing-line value) IS computed for match-tied (weekly-pool) bets -- vlr.gg's own
        estimated_start_time gives a real single closing moment, same as Tennis/Soccer. Futures bets show
        current-price-vs-placement instead, same honest degrade every other sport uses for season-long
        markets with no single closing moment.
      </p>
    </PageShell>
  );
}
