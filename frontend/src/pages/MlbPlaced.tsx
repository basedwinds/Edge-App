...
export function MlbPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "mlb"],
    queryFn: () => fetchPlacedBets(undefined, "mlb"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["mlb", "markets"] });
    queryClient.invalidateQueries({ queryKey: ["mlb", "futures"] });
  }
  ...
  return (
    <PageShell title="MLB Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        MLB bets do not auto-settle yet (no bet_settlement.py support built for MLB, same current gap
        NBA has) -- use the Won/Lost/Push buttons above once you know the outcome. A pending bet's stake
        stays locked out of MLB's own pool budget on the Recommended Bets page until it's settled.
      </p>
    </PageShell>
  );
}
