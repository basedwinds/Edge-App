  ...
  return (
    <PageShell title="Call of Duty Calibration">
        ...
        <>
          <div className="flex flex-wrap border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] mb-6 divide-x divide-[var(--color-border)]">
            ...
            <StatTile
              ...
              sublabel={`n=${data.clv_sample_size} match-tied settled bets`}
            />
          </div>
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities. No CoD market-odds backtest has been run yet, so there is no prior expectation either way -- this IS the first evidence."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        ...
        much smaller sample. Unlike the other titles, CoD has NO market-odds backtest yet -- its model was
        validated against a coin flip (64.8% walk-forward accuracy over 2,508 matches), which says the
        ratings predict, not that they beat a price. This page is the first real evidence on that question,
        so treat it as accumulating rather than settled. model_validated stays false.
      </p>
    </PageShell>
  );
}
