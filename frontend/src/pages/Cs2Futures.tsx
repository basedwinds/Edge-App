...
import { fetchCs2Futures } from "../api/markets";
import { useFuturesMarkPlaced } from "../hooks/useFuturesMarkPlaced";

/** CS2's own dedicated Futures page -- see ValorantFutures.tsx's own
 * docstring: single-event tournaments are priced by the Elo-seeded single-elim
 * Monte Carlo (flagged "approx", shown for tracking not staked); season-long
 * aggregate markets stay unpriced. */
export function Cs2Futures() {
  const { data, isLoading, isError } = useQuery({ queryKey: ["cs2", "futures"], queryFn: fetchCs2Futures });
  const onMarkPlaced = useFuturesMarkPlaced("cs2");
  const rows = data ?? [];
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex flex-wrap border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] mb-6 divide-x divide-[var(--color-border)]">
            <StatTile label="Tournaments tracked" value={String(stats.tournamentCount)} sublabel="Kalshi/Polymarket tournament-winner events" />
            <StatTile label="Markets tracked" value={String(stats.rowCount)} sublabel="one row per team in the field, per tournament" />
          </div>
          <FuturesTable rows={rows} onMarkPlaced={onMarkPlaced} sport="cs2" />
        </>
      )}

      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        No tournament-bracket model exists for CS2 yet (unlike NFL/NBA/MLB's season simulations or
