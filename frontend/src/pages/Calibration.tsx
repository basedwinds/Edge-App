  ...
  return (
    <PageShell title="Calibration">
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- consistent with this app's standing finding across every backtest: these edges mostly don't survive contact with real outcomes."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Win rate needs many settled bets before it means much -- a handful of games is mostly variance.
        Brier score and CLV are both usable with a much smaller sample, which matters while this app is
        still accumulating real placed-bet history. None of this is a claim that any model here beats the
        market (see every other disclaimer in this app) -- it's the mechanism for finding out, honestly,
        over time.
      </p>
    </PageShell>
  );
