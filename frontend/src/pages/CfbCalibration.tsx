...
export function CfbCalibration() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["bet-stats", "cfb"],
    queryFn: () => fetchBetStats("cfb"),
  });

  const brierBetter = data && data.brier_score !== null && data.market_brier_score !== null && data.brier_score < data.market_brier_score;

  return (
    <PageShell title="College Football Calibration">
      {isError && (
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- College Football is moneyline-only and the Elo matches (doesn't beat) the market, so live CLV tracking here is the real test."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        ...
        much smaller sample. None of this is a claim that the College Football model beats the market.
      </p>
    </PageShell>
  );
}
