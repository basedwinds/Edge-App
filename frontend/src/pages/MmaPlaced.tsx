...
export function MmaPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "mma"],
    queryFn: () => fetchPlacedBets(undefined, "mma"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["mma", "markets"] });
  }
  ...
  return (
    <PageShell title="MMA Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        MMA bets do not auto-settle yet (no bet_settlement.py support built for MMA, same current gap
        NBA/MLB have) -- use the Won/Lost/Push buttons above once you know the outcome. A pending bet's
        stake stays locked out of MMA's own pool budget on the Recommended Bets page until it's settled.
        CLV (closing-line value) isn't computed for MMA yet -- a UFC card has no single kickoff-equivalent
        moment (early prelims to the main event can span ~8 hours, and Kalshi keeps markets open through
        the live fight itself), so there's no reliable closing-price cutoff to check against; this shows
        current-price-vs-placement instead, same honest degrade futures bets already use.
      </p>
    </PageShell>
  );
}
