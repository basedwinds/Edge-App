  ...
  return (
    <PageShell title="Tennis Calibration">
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- consistent with this app's own backtest (see Backtests), which found the market beats this Elo baseline at every tier tested (tour/Challenger/ITF)."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        ...
        sample. This app's own fresh backtest (see Backtests) already found the market beats this Elo
        baseline at every tier tested -- none of this is a claim that any model here beats the market.
      </p>
    </PageShell>
  );
}
