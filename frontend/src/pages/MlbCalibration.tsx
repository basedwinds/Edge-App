  ...
  return (
    <PageShell title="MLB Calibration">
          ...
          {data.brier_score !== null && data.market_brier_score !== null && (
            <div className={"rounded-lg border px-4 py-3 mb-6 text-sm " + (brierBetter ? "border-[var(--color-good)]/30 bg-[var(--color-good)]/10" : "border-[var(--color-border)] bg-[var(--color-surface)]")}>
              {brierBetter
                ...
                : "On the bets placed so far, the market's implied prices have scored at least as well as the model's own probabilities -- the MLB moneyline/run-line/totals backtests already found no free historical odds source to validate against (see Backtests), so live CLV tracking here is the real test."}
            </div>
          )}
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        ...
        much smaller sample -- for MLB specifically, live CLV tracking is the ONLY real market-quality
        check available, since no free historical MLB odds source exists to backtest against (see
