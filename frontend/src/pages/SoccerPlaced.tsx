...
export function SoccerPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "soccer"],
    queryFn: () => fetchPlacedBets(undefined, "soccer"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["soccer", "markets"] });
  }
  ...
  return (
    <PageShell title="Soccer Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        Weekly-pool bets (moneyline/spread/total/btts) auto-settle once the real match result is backfilled
        from ESPN's live scoreboard -- the Won/Lost/Push buttons above are only needed for futures bets
        (league winner/relegation, which resolve at season's end) or the rare case auto-settlement can't
        resolve. A pending bet's stake stays locked out of Soccer's own pool budget on the Recommended Bets
        page until it's settled. CLV (closing-line value) IS computed for weekly-pool bets (all tied to a real
        match kickoff via SoccerMatch.estimated_start_time) -- futures bets show current-price-vs-placement
        instead, same honest degrade every other sport uses for season-long markets with no single closing
        moment.
      </p>
    </PageShell>
  );
