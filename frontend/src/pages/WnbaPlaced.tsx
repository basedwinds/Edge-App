...
export function WnbaPlaced() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["placed-bets", "wnba"],
    queryFn: () => fetchPlacedBets(undefined, "wnba"),
  });
  ...
  async function handleSettle(id: number, status: "won" | "lost" | "push" | "void") {
    ...
    queryClient.invalidateQueries({ queryKey: ["wnba", "markets"] });
  }
  ...
  return (
    <PageShell title="WNBA Placed Bets">
      {isError && (
      ...
      <p className="text-xs text-[var(--color-text-muted)] mt-4 max-w-2xl">
        WNBA moneyline auto-settles once the real final score lands. A pending bet's stake stays locked out
        of WNBA's own pool budget on the Recommended Bets page until it's settled.
      </p>
    </PageShell>
  );
}
